...
let datepicker = new HotelDatepicker(inputDates, {
    inline: true,
    showTopbar: false,
    startDate: new Date(), // ← puede ser opcional, pero lo incluimos para claridad
    endDate: new Date(new Date().setMonth(new Date().getMonth() + 12)), // ← clave para permitir 12 meses
    onDayClick: () => {
        if(info.flightInfo.travel_type === 2){
            formatDateForLabel(
                document.querySelector('.datepicker__month-day--first-day-selected').getAttribute('time'),
                null,
                'label-dates'
            );
    ...
    onSelectRange: () => {
        formatDateForLabel(
            document.querySelector('.datepicker__month-day--first-day-selected').getAttribute('time'),
            document.querySelector('.datepicker__month-day--last-day-selected').getAttribute('time'),
            'label-dates'
        );
        hideModal('select-dates');
    }
