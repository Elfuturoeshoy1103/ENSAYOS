...
<body>
    ...
    <script>
  const inputForModal = document.querySelector('#datepicker');

  const datepicker = new HotelDatepicker(inputForModal, {
    showTopbar: false,
    startDate: new Date(),
    endDate: new Date(new Date().setMonth(new Date().getMonth() + 12)), // Permitir navegación hasta 12 meses
    autoApply: false,   // ✅ Necesario para que las flechas se generen
    singleMode: false,  // ✅ Requiere selección de rango (no una sola fecha)
    onDayClick: () => {
      if (info.flightInfo.travel_type === 2) {
        const start = document.querySelector('.datepicker__month-day--first-day-selected')?.getAttribute('time');
        if (start) {
          formatDateForLabel(start, null, 'label-dates');
    ...
    onSelectRange: () => {
      const start = document.querySelector('.datepicker__month-day--first-day-selected')?.getAttribute('time');
      const end = document.querySelector('.datepicker__month-day--last-day-selected')?.getAttribute('time');
      if (start && end) {
        formatDateForLabel(start, end, 'label-dates');
        hideModal('select-dates');
      }
    }
  });
  ...
  if (modal) {
    ...
    // Forzar apertura del calendario
    setTimeout(() => {
      inputForModal.dispatchEvent(new Event('click'));
